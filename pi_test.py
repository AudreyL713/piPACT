...
class Advertiser(object):
    ...
    def __init__(self, **kwargs):
        ...
        for key, value in DEFAULT_CONFIG['advertiser'].items():
            if key in kwargs and kwargs[key]:
                ...
                print("DEBUG: Using default beacon advertiser "
                        f"configuration {key}: {value}.")
                setattr(self, key, value)
        # Create beacon
        self.__service = BeaconService(BLE_DEVICE)
        print("INFO: Initialized beacon advertiser.")
    ...
    def uuid(self, value):
        ...
        if not isinstance(value, str):
            ...
            print("DEBUG: "f"Beacon advertiser UUID set to {self.__uuid}")
        else:
            self.__uuid = value
    ...
    def advertise(self, timeout=0):
        ...
        with self.__control_file.open(mode='w') as f:
            f.write("0")
        # Start advertising
        print("INFO: Starting beacon advertiser with timeout "f"{timeout}.")
        ...
        while run:
            ...
            if timeout is not None:
                if (time.monotonic()-start_time) > timeout:
                    print("DEBUG: Beacon advertiser timed out.")
                    run = False
            self.__control_file_handle.seek(0)
            control_flag = self.__control_file_handle.read()
            if control_flag != "0":
                print("DEBUG: Beacon advertiser control flag set to ""stop.")
                run = False
        print("INFO: Stopping beacon advertiser.")
...
class Scanner(object):
    ...
    def __init__(self, **kwargs):
        ...
        for key, value in DEFAULT_CONFIG['scanner'].items():
            if key in kwargs and kwargs[key]:
                ...
                print("DEBUG: Using default beacon scanner "
                        f"configuration {key}: {value}.")
                setattr(self, key, value)
        # Create beacon
        self.__service = BeaconService(BLE_DEVICE)
        print("INFO: Initialized beacon scanner.")
    ...
    def scan(self, scan_prefix='', timeout=0, revisit=1, curr_file_id=0):
        ...
        # Start advertising
        print("INFO: "f"Starting beacon scanner with timeout {timeout}.")
        self.__control_file_handle = self.__control_file.open(mode='r+')
        run = True
        timestamps = []
        scans = []
        ...
        while run:
            scan_count += 1
            print("DEBUG: "f"Performing scan #{scan_count} at revisit "
                    f"{self.revisit}.")
            ...
            if timeout is not None:
                if (time.monotonic()-start_time) > timeout:
                    print("DEBUG: Beacon scanner timed out.")
                    run = False
            self.__control_file_handle.seek(0)
            control_flag = self.__control_file_handle.read()
            if control_flag != "0":
                print("DEBUG: Beacon scanner control flag set to stop.")
                run = False
        print("INFO: Stopping beacon scanner.")
        # Cleanup
        self.__control_file_handle.close()
        with self.__control_file.open('w') as f:
            f.write("0")
...
class Test_Scanner(object):
    """.

    Attributes:
    """

    def __init__(self, bme280, **kwargs):
        """Instance initialization.
        ...
        """
        # Beacon settings
        for key, value in DEFAULT_CONFIG['test_scanner'].items():
            if key in kwargs and kwargs[key]:
                setattr(self, key, kwargs[key])
            else:
                print("DEBUG: Using default beacon test_scanner "
                        f"configuration {key}: {value}.")
                setattr(self, key, value)
        # Create beacon
        self.__service = BeaconService(BLE_DEVICE)
        self.__bme280 = bme280
        print("INFO: Initialized beacon test_scanner.")
    ...
    def scan(self, scan_prefix='', timeout=0, revisit=1, curr_file_id=0):
        ...
        while run:
            ...
            if bool(keys):
                ...
                print(weather_data)
                print(list(data[key]))
                data[key] = list(data[key]).append(weather_data)
...
def load_config(parsed_args):
    ...
    # Load configuration YAML
        ...
        config['scanner'] = {**DEFAULT_CONFIG['scanner'],
                **config['scanner']}
        config['test_scanner'] = {**DEFAULT_CONFIG['test_scanner'],
                **config['test_scanner']}
    ...
    for key, value in parsed_args.items():
        if value is not None:
            ...
            if key in config['test_scanner']:
                config['test_scanner'][key] = value
...
def parse_args(args):
    ...
    mode_group.add_argument('-t', '--tester', action='store_true',
                            help="Beacon simultaneous scanner and advertiser mode w/ BME280.")
...
def main(args):
    ...
    # print("DEBUG: "f"Beacon configuration - {config['advertiser']}")
    # print("DEBUG: "f"Scanner configuration - {config['scanner']}")


    # Create and start beacon advertiser or scanner
    try:
        if parsed_args['advertiser']:
            print("INFO: Beacon advertiser mode selected.")
            advertiser = Advertiser(**config['advertiser'])
            advertiser.advertise()
            output = None
        elif parsed_args['scanner']:
            print("INFO: Beacon scanner mode selected.")
            scanner = Scanner(**config['scanner'])
            advertisements = scanner.scan()
            output = advertisements
        elif parsed_args['both']:
            print("INFO: Beacon simultaneous advertiser and scanner mode selected.")
            ...
            output = advertisements
        elif parsed_args['tester']:
            print("INFO: Simultaneous advertiser, scanner, and BME280 mode selected.")
